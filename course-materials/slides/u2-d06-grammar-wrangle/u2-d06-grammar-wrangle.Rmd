...
```{r message=FALSE}
hotels_df <- read_csv("data/hotels.csv")
```
...
.pull-right[
- Start with the function (a verb): `select()`
- First argument: data frame we're working with , `hotels_df`
]
...
- First argument: data frame we're working with , `hotels_df`
- Second argument: variable we want to select, `lead_time`
- Result: data frame with `r nrow(hotels_df)` rows and 1 column
]
...
.pull-left[
```{r}
select(hotels_df, hotel, lead_time)
```
]
--
.pull-right[
...
.pull-left[
- Start with the data frame `hotels_df`, and pass it to the `select()` function,
]
.pull-right[
.small[
```{r}
...
.question[
Any guesses as to why the package is called magrittr? see [Magritte](https://en.wikipedia.org/wiki/Ren%C3%A9_Magritte)
]
...
```{r error=TRUE}
ggplot(hotels_df, aes(x = hotel, fill = deposit_type)) %>%
  geom_bar()
```
...
```{r eval=FALSE}
ggplot(hotels_df,aes(x=hotel,y=deposit_type))+geom_bar()
```

`r emo::ji("white_check_mark")`

```{r eval=FALSE}
ggplot(hotels_df, aes(x = hotel, y = deposit_type)) + 
  geom_bar()
```

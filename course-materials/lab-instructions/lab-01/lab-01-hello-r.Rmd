...
This will prompt a dialogue box where you first need to enter your user name, and then your PERSONAL ACCESS TOKEN. This is a new requirement from Github - you will have to create a personal access token in order to Push your commits.
To do this, log in to Github.com, and click on your user icon on the top right.
Then click Settings.
Then scroll to the bottom of the page, and select Developer settings on the left navgation panel.
Click Personal Access Tokens and Generate new token.
Add a note such as 'data science intro course' so you know what this token will be used for.
Under 'expiration', select 90 days, so you don't have to do this again during the course.
Under 'select scopes', click 'repo', then go to bottom of page and generate token.
Copy this token to a safe place, and then you can use it in place of password to push your changes to Github.

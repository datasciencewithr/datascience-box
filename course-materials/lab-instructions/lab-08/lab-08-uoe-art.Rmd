...
-   `.Rmd`: R Markdown documents are plain text files containing **narrative** and supporting **code**
...
Go to the course GitHub organization and locate your lab6 repo, clone it in RStudio and open the R Markdown document.
...
First, install the packages, then load them with the library function.
You can do this by **running the following in your Console:**
...
Make sure you're getting the same results as before.
...
That means we must develop a list of URLs (of pages that each have 10 art pieces), and write some code that applies the `scrape_page()` function to each page, and combines the resulting data frames from each page into a single data frame with 2970 rows and 3 columns.

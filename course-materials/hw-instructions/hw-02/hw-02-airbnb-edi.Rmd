...
First, open the R Markdown document `hw-02.Rmd` and install the packages.
It will fail when it tries to install the dsbox package.
This is because it is not yet on CRAN.
To install dsbox, copy and paste the following into the Console.

install.packages("devtools") devtools::install_github("rstudio-education/dsbox")
